...
)
{
    ...
    // OslpMain:
    // - OslpMain has 1 parameter
    // - the return value of OslMain should be the return value of OslpMain
    // OslExecuteTransition:
    // - OslExecuteTransition has 0 parameters
    // - the return value of OslpMain should be the return value of OslExecuteTransition
    // OslFwpKernelSetupPhase1:
    // - should be called in the middle of OslExecuteTransition with 1 parameter
    ...
    Addr = FindPattern(Addr, 0x100,
        "\x48\x8B\xCE\xE8\x00\x00\x00\x00\x8B\xF8\x85\xC0", "xxxx????xxxx");
    if (!Addr) return EFI_NOT_FOUND;
